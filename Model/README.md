...
The `Model` project contains the domain objects that describe a full wellbore architecture, including probabilistic drilling properties, structural elements, identity and feature metadata, and helper enumerations. It targets `net8.0` and depends on OSDC drilling libraries for unit handling, data management, and statistical distributions.
...
`WellBoreArchitecture` is the aggregate root. It encapsulates metadata, identity and feature assignments, wellhead information, above-ground fluids, surface sections, casing sections, and the optional final `OpenHoleSection`. Each casing owns its `CasingSectionSizeTable`, which describes borehole diameters through that casing interval; open-hole data is not duplicated on individual casings. Nested construction objects expose a `ComponentID` UUID so callers can address them without relying on array position; legacy serialized objects may omit it and the service materializes a deterministic value. Identity definitions and feature categories/options are user-manageable catalogs; assignments store stable UUID references to those definitions.

`WellBoreArchitectureBatchExport.cs` defines the schema-versioned backup contract. A backup contains all or a selected ordered set of complete architectures plus only the identity definitions, feature categories, and feature options referenced by those records. Restore requests explicitly choose UUID-conflict and catalogue policies; normalized-name mapping between different UUIDs is disabled unless `AllowNormalizedNameMapping` is explicitly selected.
...
- `ModelTest/ModelTest.csproj` depends on the model and exercises the aggregate calculation rule.
...
{
    Name = "Demo well",
    WellHead = new WellHead { Depth = new GaussianDrillingProperty() },
};
