...
`WellBoreArchitecture` is the aggregate root. It encapsulates metadata, identity and feature assignments, wellhead information, above-ground fluids, surface sections, and casing sections. Nested construction objects expose a `ComponentID` UUID so callers can address them without relying on array position; legacy serialized objects may omit it and the service materializes a deterministic value. Identity definitions and feature categories/options are user-manageable catalogs; assignments store stable UUID references to those definitions.
