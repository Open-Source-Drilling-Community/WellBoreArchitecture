...
- The management menu includes a Backup / Restore page for schema-version-2 JSON backups of all or selected architectures. Restore uses explicit conflict/catalogue policies and one SQLite transaction. Exact UUID matching is the safe catalogue default; normalized-name matching requires explicit consent.
